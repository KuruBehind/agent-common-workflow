...
### 다른 레포의 스킬을 조건부로 쓰는 법 — 사용자 스코프 배포

> ⚠️ 2026-09-09 정정: 이전 버전은 여기서 "`@import`(상시 강제 로드)가 사실상 유일한 방법"이라고 서술했으나 **틀렸습니다.**

두 도구 모두 홈 디렉토리 스킬 경로를 지원합니다. 여기에 복사하면 어느 워크스페이스에서든 조건부로 잡힙니다.
...
`agent-common-workflow/tools/sync-agent-skills.sh`가 `skills/`를 양쪽에 복사합니다. 효과:

* 상시 강제 로드 → **조건부 로드** (2026-09-09 실측: 상시 1,778줄 중 1,406줄이 조건부로 전환)
* **크로스레포 파일 접근이 불필요** — 다른 레포 경로를 읽으려 하면 권한 프롬프트가 뜨는데, 그게 사라진다
* `--add-dir` 플래그를 세션마다 켤 필요 없음

트레이드오프: 사용자 스코프는 git이 아니라 머신 로컬이라, 공통 레포를 `git pull` 한 뒤 sync를 다시 돌려야 한다.
sync를 못 돌린 사람이 스킬을 조용히 잃지 않도록, **프로세스 척추**(`workflow`·`communication`·`code-conventions`)만 `CLAUDE.md`에서 `@import`로 상시 로드해 둔다.

### 레포 고유 스킬은 `.agents/` 가 정본

벤더 중립 경로(`.agents/skills/`)를 정본으로 두고, `.claude/skills/`는 sync 스크립트가 만드는 생성물로 둔다. **둘 다 커밋한다** — 생성물을 gitignore 하면 클론 직후 sync를 안 돌린 사람이 스킬을 통째로, 그것도 조용히 잃는다.
